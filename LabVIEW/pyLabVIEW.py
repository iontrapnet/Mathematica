...
#VIDir = os.path.abspath(inspect.stack()[0][1])
VIDir = os.path.realpath(__file__)
VIDir = os.path.dirname(VIDir)
#sys.path.append(VIDir)
LabVIEWInterop = clr.AddReference('LabVIEW')
from LabVIEW import ApplicationClass,VirtualInstrument

AppCls = ApplicationClass()
VIDir += '/'
VIDir = os.path.normcase(VIDir)
_LoadVI = {}
def LoadVI(name, option=16, resv=True):
    if name in _LoadVI:
        return _LoadVI[name]
